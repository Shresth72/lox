        ...
        loop {
            ...
            break match started {
                ...
                Started::Number => {
                    ...
                    literal = match (dotted.next(), dotted.next(), dotted.next()) {
                        (Some(one), Some(two), Some(_)) => &literal[..one.len() + 1 + two.len()],
                        (Some(one), Some(two), _) if two.is_empty() => &literal[..one.len()],
                        _ => literal,
                    };
